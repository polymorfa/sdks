...
- Added the typed `usage.recorded` webhook payload from the pending usage
  contract. It carries a stable record ID and increasing revision when a later
  observation corrects the record; usage is measured but not charged.

- `constructWebhookEvent` accepts project-scoped deliveries with an empty
  `session`, such as `customer.*` events, which it previously
