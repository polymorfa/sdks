...
async function platformServer(): Promise<{ client: PlatformClient; requests: RecordedRequest[] }> {
  const server = await startTestServer(() => ({
    headers: { "content-type": "application/json", "x-request-id": "req_platform" },
    body: '{"data":{}}',
  }));
  servers.push(server);
  return {
    requests: server.requests,
    client: new PlatformClient({ apiKey: "pmfa_platform", baseUrl: server.url, maxNetworkRetries: 0 }),
  };
}

describe("PlatformClient credentials", () => {
  it("rejects client and project tokens before issuing a request", () => {
    expect(() => new PlatformClient({ apiKey: "pmfa_ct_browser" })).toThrow(PolymorfaConfigurationError);
    expect(() => new PlatformClient({ apiKey: "pmfa_pt_project" })).toThrow(PolymorfaConfigurationError);
  });
});
...
describe("PlatformClient organizations and projects", () => {
  it("retrieves and updates the active organization", async () => {
    ...
    await client.organizations.update({ name: "Support Team", timezone: "Asia/Beirut" });
    expect(requests.map(({ method, path }) => `${method} ${path}`)).toEqual([
      "GET /v1/organization",
      "PATCH /v1/organization",
    ]);
    expect(requests[1]?.body).toBe('{"name":"Support Team","timezone":"Asia/Beirut"}');
  });
  ...
  it("lists and creates projects", async () => {
    ...
    const created = await client.projects.create(
      { name: "Support", icon: { type: "emoji", value: "💬" }, defaultTier: "standard" },
  ...
  it("maps production enrollment request, approval, and cancellation", async () => {
    ...
    await client.projects.requestProductionEnrollment("project/a", { business });
    await client.projects.approveProductionEnrollment("project/a", "operation/b");
    await client.projects.cancelProductionEnrollment("project/a", "operation/b");
...
describe("PlatformClient sessions", () => {
  ...
  it("maps stop, delete, tier override, and testing-session operations", async () => {
    ...
    await client.sessions.setTierOverride("session/a", { projectId: "project_1", tierOverride: "pro" });
    await client.sessions.createTesting({ projectId: "project_1", name: "Demo", country: "US" });
    ...
    expect(requests[2]?.body).toBe('{"projectId":"project_1","tierOverride":"pro"}');
  });
});
